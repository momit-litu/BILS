...
var Main = function() {
	...
	var runModuleTools = function() {
		...
		$('.panel-tools .panel-refresh').on('click', function(e) {
			...
			el.block({
				...
				message: '<img src="http://localhost/survey/public/assets/images/loading.gif" /> Just a moment...',
				css: {
					border: 'none',
					color: '#333',
					background: 'none'
	...
	var setColorScheme = function() {
		$('.icons-color a').on('click', function() {
			...
			$('#skin_color').attr("href", "public/assets/css/theme_" + $(this).attr('id') + ".css");
	...
	var runActivateLess = function() {
		...
		if($('#skin_color').attr("rel") == "stylesheet") {
			$('#skin_color').attr("rel", "stylesheet/less").attr("href", "public/assets/less/styles.less");
